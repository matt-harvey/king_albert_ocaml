type t =
  | Foundation of Card.t
  | Column of Card.t list
  | Reserve of Card.t Option.t

let can_give (position : t) : bool = match position with
  | Foundation(_) -> false
  | Column([]) -> false
  | Column(_ :: _) -> true
...
let can_receive (position : t) (rank, suit) : bool = match (position, rank, suit) with
  | (Foundation((top_rank, top_suit)), rank, suit) ->
      suit = top_suit && rank = succ top_rank
...
let give (position : t) : t * Card.t option = match position with
  | Column(top_card :: rest) ->
      (Column(rest), Some(top_card))
...
let receive (position : t) (card : Card.t) : t = match (position, card) with
