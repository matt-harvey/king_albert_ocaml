...
let positions (board : t) : Position.t list = board |> CharMap.bindings |> List.split |> snd

let foundations (board : t) : Position.t list =
  board |> positions |> List.filter (fun v -> match v with Position.Foundation (_, _) -> true | _ -> false)

let columns (board : t) : Position.t list =
  board |> positions |> List.filter (fun v -> match v with Position.Column _ -> true | _ -> false)

let reserves (board : t) : Position.t list =
  board |> positions |> List.filter (fun v -> match v with Position.Reserve _ -> true | _ -> false)
